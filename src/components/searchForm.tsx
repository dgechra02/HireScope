...
export default function SearchForm() {
  const { searchedInput, setSearchedInput } = useCustomHook();
  const [suggestion, setSuggestion] = useState([]);

  useEffect(() => {
    async function getSuggestion() {
      const res = await fetch(
        `http://localhost:3000/api/jobSearch/suggestion?q=${searchedInput}`
      );
    ...
    <div className="relative">
      ...
      >
        <input
          ...
          autoComplete="on"
          className="p-1.5 outline-none w-150 text-lg"
          value={searchedInput}
